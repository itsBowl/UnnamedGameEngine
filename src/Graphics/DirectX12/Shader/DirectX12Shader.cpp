...
namespace EngineCore
{
    ...
    DirectX12Shader::DirectX12Shader(ID3D12Device* d, const std::string& fp)
    {
        ...
        {
            if (e.second == ".hlsl_ps") fragPath = std::wstring(e.first.begin(), e.first.end());
            else if (e.second == ".hlsl_vs") vertPath = std::wstring(e.first.begin(), e.first.end());
        }

        if (!compileStage(d, vertPath, "VSMain", "vs_5_1", vertexBlob))
        {
            shaderErrorCode = ShaderErrors::SHADER_FAILED_COMPILE;
            Log::error(LOGGER_TAG, "Vertex Shader Compile Failed: ", vertPath);
            return;
        }
        ...
        {
            shaderErrorCode = ShaderErrors::SHADER_FAILED_COMPILE;
            Log::error(LOGGER_TAG, "Fragment Shader Compile Failed: ", fragPath);
            return;
        }

        if (!createRootSigniture(d))
        {
                                        ...
                                        const std::string& target, ComPtr<ID3DBlob>& out)
    {
        ...
        if (FAILED(res))
        {
            if (err)
            {
                Log::error(LOGGER_TAG, "HLSL ERROR: ", static_cast<const char*>(err->GetBufferPointer()));
            }
            else
            {
    ...
    bool DirectX12Shader::createRootSigniture(ID3D12Device* d)
    {
        ...
        ComPtr<ID3DBlob> sig;
        ComPtr<ID3DBlob> err;
        HRESULT res = D3D12SerializeVersionedRootSignature(&rsd, &sig, &err);

        if (FAILED(res))
        {
            if (err)
            {
                Log::error(LOGGER_TAG, "Root Signiture serialisation error: ", static_cast<const char*>(err->GetBufferPointer()));
            }
            else
            {
